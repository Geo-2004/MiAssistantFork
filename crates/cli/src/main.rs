...
#[derive(Parser)]
#[command(author, version, about = "MAF (MiAssistantFork) – Xiaomi Recovery Flash & Rescue")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
    /// Enable verbose logs
...
enum Commands {
    ...
    Info,
    Adb { cmd: String },
    Sideload { file: String, #[arg(short, long)] validate: Option<String>, #[arg(long)] resume: bool },
    Md5 { file: String },
    ListRoms,
    Flash { file: String, #[arg(long)] yes: bool },
    FormatData,
    Reboot,
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Detect => {
            let _dev = usb::find_first_adb().context("No device found. Put device in recovery/MiAssistant mode and connect via USB.")?;
        ...
        Commands::Adb { cmd } => {
            ...
            let mut t = adb::AdbTransport { dev: &mut dev, timeout_ms: 5_000 };
            t.connect().context("ADB connect failed")?;
            let out = t.simple_command(&cmd).with_context(|| format!("Failed ADB command: {}", cmd))?;
            println!("{}", out);
        }
        Commands::Sideload { file, validate, resume } => {
            ...
            let token = if let Some(tok) = validate {
                ...
                let res = validate::Validator::new()?.validate(&info, &md5sum, true).context("Xiaomi validation failed (auth/token)")?;
                match res { validate::ValidationResult::FlashToken { token, .. } => token, _ => return Err(anyhow::anyhow!("Expected flash token (Validate) in Xiaomi response")) }
            };
            sideload::sideload_resumable(&mut t, &file, &token, &cancel, resume).context("ADB sideload failed")?;
        }
        ...
        Commands::Flash { file, yes } => {
            ...
            if let validate::ValidationResult::FlashToken { token, erase } = res {
                ...
                sideload::sideload_resumable(&mut t, &file, &token, &cancel, false).context("ADB sideload failed")?;
            } else { return Err(anyhow::anyhow!("Expected flash token (Validate) in Xiaomi response")); }
        }
        Commands::FormatData => {
            let mut dev = usb::find_first_adb().context("No device found. Put device in recovery/MiAssistant mode and connect via USB.")?;
            let mut t = adb::AdbTransport { dev: &mut dev, timeout_ms: 5_000 };
            t.connect().context("ADB connect failed")?;
            let out = t.simple_command("format-data:").context("Format data failed")?;
            println!("{}", out);
